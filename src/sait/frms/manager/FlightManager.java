...
public class FlightManager {
	...
	/**
	 * The method returns an ArrayList of any matching Flight objects. If no matches
	 * are found, the list control will be empty.
	 * 
		...
		for (Flight f : flights) {
			if (f.getFrom().equals(from) && f.getTo().equals(to)) {
				if (weekday.equals(WEEKDAY_ANY)) {
					foundFlights.add(new Flight(f.getCode(), f.getFrom(), f.getTo(), f.getWeekday(), f.getTime(),
							f.getSeats(), f.getCostPerSeat()));
	...
	public String[] getDayArray() {
		String[] day = { WEEKDAY_ANY, WEEKDAY_SUNDAY, WEEKDAY_MONDAY, WEEKDAY_TUESDAY, WEEKDAY_WEDNESDAY,
